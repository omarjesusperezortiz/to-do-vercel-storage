<template>
  ...
  <div class="form-container">
    ...
    <div class="form-group task-button">
      <button @click="changeDescriptionTask(id)">
...
const changeDescriptionTask = async (id) => {
  ...
  errorMessage.value = 'Actualizando...';
  showErrorMessage.value = true;
  try {
    await taskStore.updateTask(id, title.value, description.value)
    title.value = ''
    description.value = ''
    emit('close');
  } catch (error) {
    ...
    showErrorMessage.value = true;
    errorMessage.value = 'Error al editar tarea. Intente de nuevo.';
  } finally {
    showErrorMessage.value = false;
    errorMessage.value = '';
